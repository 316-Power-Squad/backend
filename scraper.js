  ...
  return new Promise((resolve, reject) => {
        ...
        if (!err && resp.statusCode == 200) {
            ...
            $('a', '.headleague').each(function() {
            	let splitRegion = $(this).text().split(' ');
            	let region = splitRegion.splice(1, splitRegion.length).join(' ');
            	if (splitRegion.includes('DI')) {
            		db.get().query(`INSERT INTO Team (name, gender, region) values (?, ?, ?)`, [team, 'mens', region], () => {});
            		db.get().query(`INSERT INTO Team (name, gender, region) values (?, ?, ?)`, [team, 'womens', region], () => {});
            		console.log(region);
            	}	
            });
            db.disconnect();
