...
const insertMeet = function(meet) {
  db.get().query(`INSERT INTO Meet (name) values (?)`, [meet], () => {});
};

const insertParticipates = function(meet, team, gender, place) {
  let query = `INSERT INTO Participates (team_id, meet_id, placement) values ((SELECT ID FROM Team WHERE name=\'`.concat(team).concat(`\' AND gender=\'`.concat(gender).concat(`\'), (SELECT ID FROM Meet where name=\'`.concat(meet).concat(`\'), `.concat(place).concat(`)`))));
  //console.log(query);
  db
    .get()
    .query(
...
const insertResults = async () => {
  ...
  for (let meet of results.keys()) {
    ...
    for (let mensTeam of r[0].keys()) {
      ...
      //console.log(r[0].get(mensTeam));
      insertParticipates(meet, mensTeam, "mens", r[0].get(mensTeam));
    }
    for (let womensTeam of r[1].keys()) {
      //console.log(womensTeam);
      //console.log(r[1].get(womensTeam));
      insertParticipates(meet, womensTeam, "womens", r[1].get(womensTeam));
    }
  }
};
