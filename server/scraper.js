...
const insertTeam = async (school) => {
  console.log(`Inserting mens and womens team for school ${school}`);
  return new Promise((resolve, reject) => {
    db.get().query(
      `INSERT INTO Team (school_id, gender) values ((SELECT id FROM School WHERE School.name=?), mens)`,
      [school]
    );
    db.get().query(
      `INSERT INTO Team (school_id, gender) values ((SELECT id FROM School WHERE School.name=?), womens)`,
      [school]
    );
  })
}
