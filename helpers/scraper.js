var request = require('request'), cheerio = require('cheerio'), urls = [];
...
request('https://en.wikipedia.org/wiki/List_of_NCAA_Division_I_institutions', function(err, resp, body) {
	if (!err && resp.statusCode == 200) {
		...
		$('th', '.sortable').each(function() {
			var text = $(this).text();
			if (text.split(" ").length > 1 && notTitle(text)) {
				console.log(text);
				count++;
			}
		});
