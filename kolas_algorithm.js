...
let meets = {
  ...
  sundodger_invitational: [
    '2017-09-16',
    ['Washington', 'Oregon State', 'Nevada', 'Creighton'],
  ],
  coast_to_coast_battle_in_beantown: [
    '2017-09-22',
    [
      'Providence',
      'Indiana',
  ...
  panorama_farms_invitational: [
    '2017-09-23',
    ['Virginia', 'Yale', 'SMU', 'William and Mary', 'Bucknell', 'Charlotte'],
  ],
  roy_griak_invitational: [
    '2017-09-23',
    [
      'San Francisco',
      'Iowa State',
  ...
  dellinger_cal_poly_v_uc_santa_barbara: [
    '2017-09-29',
    ['Cal Poly', 'UC Santa Barbara'],
  ],
  dellinger_cal_poly_v_washington: ['2017-09-29', ['Washington', 'Cal Poly']],
  dellinger_oregon_state_v_portland: [
    '2017-09-29',
    ['Portland', 'Oregon State'],
  ],
  dellinger_oregon_state_v_stanford: [
    '2017-09-29',
    ['Stanford', 'Oregon State'],
  ],
  dellinger_oregon_state_v_uc_santa_barbara: [
    '2017-09-29',
    ['Oregon State', 'UC Santa Barbara'],
  ],
  dellinger_oregon_state_v_washington: [
    '2017-09-29',
  ...
  dellinger_oregon_v_uc_santa_barbara: [
    '2017-09-29',
  ...
  dellinger_portland_v_uc_santa_barbara: [
    '2017-09-29',
    ['Portland', 'UC Santa Barbara'],
  ],
  dellinger_portland_v_washington: ['2017-09-29', ['Washington', 'Portland']],
  dellinger_stanford_v_uc_santa_barbara: [
    '2017-09-29',
    ['Stanford', 'UC Santa Barbara'],
  ],
  dellinger_stanford_v_washington: ['2017-09-29', ['Stanford', 'Washington']],
  dellinger_uc_santa_barbara_v_washington: [
    '2017-09-29',
    ['Washington', 'UC Santa Barbara'],
  ],
  joe_piane_notre_dame_invitational: [
    '2017-09-29',
    [
      'Colorado',
      'New Mexico',
  ...
  nuttycombe_wisconsin_invitational: [
    '2017-10-13',
    [
      'New Mexico',
      'San Francisco',
  ...
  ivy_league_heptagonal_championships: [
    '2017-10-27',
    ['Columbia', 'Dartmouth', 'Yale', 'Princeton', 'Cornell', 'Brown', 'Penn'],
  ],
  mountain_west_championships: [
    '2017-10-27',
    [
      'New Mexico',
      'Boise State',
  ...
  west_coast_conference: [
    '2017-10-27',
    ['San Francisco', 'BYU', 'Loyola Marymount', 'Portland'],
  ],
  big_12_championships: [
    '2017-10-28',
    [
      'Iowa State',
      'Oklahoma State',
  ...
  big_east_championships: [
    '2017-10-28',
    ['Villanova', 'Providence', 'Georgetown', 'Marquette', 'Creighton'],
  ],
  big_sky_championships: [
    '2017-10-28',
    [
      'Northern Arizona',
      'Montana State',
  ...
  if (!awardpoints) {
    d2.messages.push(
      '\n' +
        ...
        ' ' +
        region +
        ') is selected, gives no points (pushed in)\n'
    );
...
let getEligible = function(data) {
  ...
  for (let rname in regionals) {
    ...
    if (data.curr_inds[rname] < l.length) {
      ...
      if (
        data.teamsin.length < 30 &&
        !data.pushes_used[rname] &&
        data.curr_inds[rname] + 1 < l.length
      ) {
...
let resolveTies = function(oldtied, d) {
  ...
  //Head to Head
  d.messages.push(
    '  Breaking ties between ' + tied.join() + ' using head-to-head\n'
  );
  let lost = [];
  ...
  for (let i = 0; i < tied.length; i++) {
    ...
    for (let j = i + 1; j < tied.length; j++) {
      ...
      if (relevant.length == 0) {
        d.messages.push(
          '  No head-to-head tiebreaker for ' + ti + ' and ' + tj + '\n'
        );
        continue;
      } else if (score < 0) {
        d.messages.push(
          '  ' + ti + ' loses head-to-head vs ' + tj + ', is not selected'
        );
        lost.push(ti);
      } else if (score < 0) {
        d.messages.push(
          '  ' + tj + ' loses head-to-head vs ' + ti + ', is not selected'
        );
        lost.push(tj);
      } else {
        ...
        if (dscore < 0) {
          d.messages.push(
            '  ' +
              ti +
              ' loses head-to-head vs ' +
              tj +
              ' (most recent matchup), is not selected'
          );
          lost.push(ti);
        } else {
          d.messages.push(
  ...
  if (tied.length == 1) {
    ...
    d.messages.push(
      '  Head-to-head tiebreaker eliminated all teams; ignoring.\n'
    );
    tied = oldtied;
  }
  ...
  // Common Opponents
  d.messages.push(
    '  Breaking ties between ' + tied.join() + ' using common opponents\n'
  );
  lost = [];
  for (let i = 0; i < tied.length; i++) {
    let ti = tied[i];
      ...
      if (common_opps.length == 0) {
        ...
        if (win_percent_i > win_percent_j) {
          d.messages.push(
            '  ' +
              tj +
              ' loses common opponents vs ' +
              ti +
              ', is not selected (win percentages: ' +
              win_percent_i +
              ', ' +
              win_percent_j +
              ')'
  ...
  if (tied.length == 1) {
    ...
    d.messages.push(
      '  Common opponent tiebreaker eliminated all teams; ignoring.\n'
    );
  ...
  d.messages.push(
    '  Breaking ties between ' + tied + ' using highest place at regionals.\n'
  );
  ...
  if (newtied.length == 1) {
    d.messages.push(
      '  Broke tie by selecting ' + newtied[0] + ' (highest regional finish).\n'
    );
    return newtied[0];
  } else if (newtied.length == 0) {
    //should not happen
    console.log(
      'newtied.length == 0 for some reason during 1st score at regionals'
    );
  } else {
    tied = newtied;
  }
  ...
  d.messages.push(
    '  Breaking ties between ' +
      tied +
      ' using closest score to 2nd-place in region.\n'
  );
  ...
  if (newtied.length == 1) {
    d.messages.push(
      '  Broke tie by selecting ' +
        newtied[0] +
        ' (closest score to regional 2nd-place).\n'
    );
    return newtied[0];
  } else if (newtied.length == 0) {
    //should not happen
    console.log(
      'newtied.length == 0 for some reason during 2nd score at regionals'
    );
  } else {
    tied = newtied;
  }
...
let printPoints = function(el, data) {
  ...
  let pts_keys = Object.keys(pts_to_teams)
    .sort(function(a, b) {
  ...
  for (let i = 0; i < pts_keys.length; i++) {
    ...
    data.messages.push(
      '  ' +
        pts +
        ' '.repeat(3 - pts.toString().length) +
        teamlist.join() +
        '\n'
    );
  }
  data.messages.push('  Others with points: ');
  let all_by_pts = Object.keys(data.points)
    .sort(function(a, b) {
      return data.points[a] - data.points[b];
    })
    .reverse();
